...
ide_dir="${RUSTROVER_HOME:-/home/sondre/.local/share/JetBrains/RustRover-2026.2.1}"
...
if [[ ! -f "$ide_dir/build.txt" ]]; then
    printf 'RustRover was not found at %s\n' "$ide_dir" >&2
    printf 'Set RUSTROVER_HOME to the RustRover installation directory.\n' >&2
    exit 1
fi
...
"$ide_dir/jbr/bin/javac" --release 25 -classpath "$classpath" -d "$classes_dir" "${sources[@]}"
